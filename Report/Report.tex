...
Using the nltk library in Python, we convert every word to its constitutive arpabet phonetics using \ref{Ph.py}. It gives the following output for the words - f', 'see', 'sea', 'compute', 'comput', 'cat'. Only 'comput' fails because it isn't a real word
\begin{verbatim}
['EH1', 'F']
['S', 'IY1']
['S', 'IY1']
...
\begin{itemize}
	\item Getting the transition probabilities between phonemes- I'll do that next
	\item Getting a feature space for each phoneme.
	\item Code to extract all the output from the subtitles file.
